...
- Documentation 홈/목차, Beginner's guide 전체, 기본 메커니즘 문서 번역 패치 적용 및 화면 검증 완료
- Documentation 고급 메커니즘 화면 확인 가능 문서(Hacking algorithms, Offline scripts/Bonus Time, IPvGO, Darkweb Network) 번역 패치 적용 및 화면 검증 완료
- NS/API 문서 트리의 NS 개요, NS.args/ScriptArg, API 공통 구조, package 대표 설명문 번역 패치 적용 및 화면 검증 완료
- API 문서 반복 구조 대응을 위해 패처에 `regexReplace` operation 추가

자세한 실험 진행 상황과 스크린샷은 [`docs/08_experiment_log.md`](docs/08_experiment_log.md)를 본다. NS/API 문서 트리 작업 방식과 검증 기록은 [`docs/09_api_documentation_tree.md`](docs/09_api_documentation_tree.md)에 분리했다.
...
- `docs/09_api_documentation_tree.md`: NS/API 문서 트리 작업 방식과 검증 기록
- `docs/changelog.md`: 변경 기록

## 다음 실험 후보

1. NS/API 하위 문서 설명문 확장: package 표, 고빈도 NS 메서드, namespace 문서.
2. Documentation 진행도 잠금 고급 문서 해금 후 번역 및 자료/마이그레이션 문서 확장.
3. Dark Net 인증/상세 모달 잔여(`Logs scraped via`, `Hint:`) 별도 보강.
4. Faction 긴 lore/rumor 문구 sweep.
5. 개별 Augmentation lore/Grafting 구매 화면 보강.
6. Hacknet 관련 나머지 설명/툴팁.
7. 새 화면 잔여 발견 시 보정 manifest 추가.
