...
- Monaco 에디터 예외 CSS가 코드 본문에만 적용되는지
...
- Monaco 코드 본문은 기존 코딩 폰트 계열로 유지되는지
- 한글/영문 혼합 줄에서 baseline이나 글자 폭이 크게 어긋나지 않는지

## Monaco 코드 에디터 예외 처리

전체 UI NeoDunggeunmo 적용은 유지하되, Script Editor의 Monaco 코드 본문은 기존 코딩 폰트 계열로 되돌렸다.

적용 방식:

- `patches/font_monaco_exception.json`
- `index.html`에 `--bb-kr-code-font-family: JetBrainsMono, "Courier New", monospace` 추가
- `.monaco-editor`, `.monaco-diff-editor` 내부에만 코드 폰트 override 적용

검증 결과:

- dry-run, apply, 재 dry-run `already-applied` 확인
- Script Editor 화면에서 코드 본문이 NeoDunggeunmo가 아닌 기존 코딩 폰트 계열로 표시됨
- 일반 UI와 floating window는 NeoDunggeunmo 유지
